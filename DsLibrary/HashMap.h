#ifndef __HASH_MAP_GENERIC_H__
#define __HASH_MAP_GENERIC_H__

#include <stddef.h>  /* size_t */

#include "../DsLibrary/HiddenList.h"
#define PRIME 1
#define NOT_PRIME 0
#define ZERO 0
#define NOT_EQUALS 0
...
typedef enum Map_Result {
	MAP_SUCCESS,
	MAP_UN_INITIALIZED_ERROR, 		/**< Uninitialized map error 	*/
...
typedef size_t (*PtrFuncHashFunction)(void* _key);
typedef int (*PtrFuncEqualityFunction)(void* _firstKey, void* _secondKey);
typedef int	(*PtrFuncKeyValueActionFunction)(const void* _key, void* _value, void* _context);
typedef void (*PtrFuncKeyDestroy)(void* _key);
typedef void (*PtrFuncValDestroy)(void* _value);


/** 
 * @brief Create a new hash map with given capcity and key characteristics.
 * @param[in] _capacity - Expected max capacity 
 * 						  shall be rounded to nearest larger prime number, must be bigger than 1.
 ...
 */
HashMap* HashMap_Create(size_t _capacity, PtrFuncHashFunction _hashFunc, PtrFuncEqualityFunction _keysEqualFunc);
 ...
 */
void HashMap_Destroy(HashMap** _map, PtrFuncKeyDestroy _keyDestroy, PtrFuncValDestroy _valDestroy);
...
List* GetList(HashMap* _hash, size_t _index);
 ...
 */
size_t HashMap_ForEach(const HashMap* _map, PtrFuncKeyValueActionFunction _action, void* _context);
